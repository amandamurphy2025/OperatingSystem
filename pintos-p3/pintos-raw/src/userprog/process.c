...
static bool
init_cmd_line (uint8_t *kpage, uint8_t *upage, const char *cmd,
               void **esp) 
{
  ...
  for (token = strtok_r (kpage + PGSIZE - cmd_size - 1, " ", &save_ptr); token != NULL;
      ...
      if (page->frame != NULL)
        {
          ...
          success = init_cmd_line (page->frame->base, page->addr, cmd, esp);
